...
export interface PromtParams extends INodeFunctionBaseParams {
	config: {
		text: string;
		url: string;
...
export const promt = createNodeDescriptor({
	type: "promt",
	defaultLabel: "Promt",
	fields: [
		{
	...
	function: async ({ cognigy, config }: PromtParams) => {
		...
		const jambonzPayload: PromtJambonz = {
			text,
			url,
			config: {
